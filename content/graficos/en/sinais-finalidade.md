...
titulo: "Use-case signals"
subtitulo: "The task that grew most between two non-overlapping snapshots, and the model that gained most within it."
tipo: texto
modos: []
indicadores:
...
oQueNaoMostra: >-
  The source publishes only the current week's snapshot and keeps no history, so the series starts on the day the archive began. Earlier weeks cannot be rebuilt, and no number is estimated to fill the gap. A week of growth in a task can also be a single large customer testing something.
---
