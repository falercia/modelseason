...
**Em pauta.** Na segunda, na quarta e na sexta, `pipeline/pauta.py` lê fontes externas (blogs da OpenAI, do Google e da Anthropic, notas de lançamento da DeepSeek, pesos novos de 16 laboratórios no Hugging Face, entre eles os chineses que lideram o tráfego, NIST, Comissão Europeia, a tramitação do PL 2338/2023 na Câmara, TechCrunch, MIT Technology Review, The Verge, Ars Technica, Simon Willison, Tecnoblog, Techmeme e Hacker News), agrupa por assunto com IA e dá nota por fórmula: peso das fontes, pontos no Hacker News, categoria e share dos laboratórios citados. Os três assuntos de maior nota ganham resumo em português e inglês escrito só com o que as fontes publicaram, e número que não está nas fontes derruba o texto. A coleta cobre o período desde a edição anterior (54 horas na quarta e na sexta, 78 na segunda). O workflow `pauta.yml` abre um PR com `data/pauta/AAAA-MM-DD.json`, e só o merge publica. Ao abrir a edição nova, a anterior que ficou sem merge é fechada, então há uma pauta aberta por vez. Preço só vira notícia quando o novo valor se mantém por duas fotos, para não publicar oscilação de roteamento. A ordem segue uma fórmula fixa de relevância, declarada no próprio código. `data/pauta/destaques.json` lista temas que o editor está acompanhando (`termo`, `ate`, `bonus` opcional): assunto que cita o termo ganha o bônus na nota e sai marcado como destaque no JSON e no PR. Item que cita um destaque nunca cai no corte de itens enviados à IA. É um empurrão na ordem, não um furo nas travas: categoria, repetição e conferência de números continuam valendo.
