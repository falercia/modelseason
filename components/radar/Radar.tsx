...
import type { EdicaoRadar, EventoRadar } from '@/lib/tipos';
...
export function Edicao({ ed, I, paginas, titulo }: { ed: EdicaoRadar; I: Idioma; paginas: Set<string>; titulo?: string }) {
  ...
  return (
    <section className="rd-ed" aria-label={titulo ?? tituloEdicao(ed, I)}>
      {titulo && <h2>{titulo}</h2>}
      {!p ? <p className="rd-vazio">{t({ pt: 'Nada relevante mudou no catálogo nem no tráfego neste dia.', en: 'Nothing relevant changed in the catalog or in traffic on this day.' })}</p> : (
        <>
          <Evento ev={p} dia={ed.dia} I={I} paginas={paginas} destaque />
          {cartoes.length > 0 && (
